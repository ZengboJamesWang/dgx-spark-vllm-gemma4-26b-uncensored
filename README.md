...
- **Memory-optimized settings**: Tuned `--gpu-memory-utilization 0.60` with full `--max-model-len 262000` and `--max-num-seqs 16` — achieves same 45+ tok/s while using ~47GB GPU memory instead of ~100GB (see [Recommended Settings](#recommended-settings))
...
**Current configuration**: `--gpu-memory-utilization 0.60` | `--max-model-len 262000` | **`--max-num-seqs 16`**

> **Why 16 concurrent sequences?** This limits KV cache memory to ~15GB (down from ~48GB with 128), keeping total GPU usage at ~47GB. More than enough for OpenClaw agents while maintaining full 262K context.
...
> **Note on settings**: The command above uses `--gpu-memory-utilization 0.60`, `--max-model-len 262000`, and `--max-num-seqs 16`, which differ from the [HuggingFace model card](https://huggingface.co/AEON-7/Gemma-4-26B-A4B-it-Uncensored-NVFP4) defaults (`0.85`, `65536`, and `256`). After extensive testing on DGX Spark, we found `0.60` with full `262000` context and `16` concurrent sequences provides the optimal balance — using ~47GB GPU memory (vs ~100GB with the card's defaults) while maintaining the full context window and same 45+ tok/s performance.
...
- Set `--gpu-memory-utilization 0.60` with `--max-num-seqs 16` (262K context fits comfortably)
...
| Setting | Value | Why |
|---------|-------|-----|
| `--gpu-memory-utilization` | **0.60** | Sweet spot: ~77GB budget, actual usage ~47GB. Full 262K context with 16 concurrent sequences. |
